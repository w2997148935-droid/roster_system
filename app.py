...
def init_db():
    with app.app_context():
        ...
        if not User.query.filter_by(username='admin').first():
            ...
            db.session.commit()
            print("✅ 数据库就绪，默认管理员: admin / admin123")
...
def dashboard():
    ...
    return render_template('dashboard.html', count=my_shifts_count, sent_reqs=my_sent_requests, recv_reqs=my_received_requests)
...
def admin_generate():
    ...
    group = request.form.get('group') # 可选
